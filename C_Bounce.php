...
$query = "SELECT * FROM forum_members WHERE contact_email='" . $email_check . "';" ;
...
if (mysqli_num_rows($result) > 0) {
		...
		$notification1 = "We could not confirm your Contact Email. Please try reentering it from 'Change Personal Info'. If Sidney Jane continues to rage out of control, feel free to tattle to sidney.jane@forum.sandboxsalinas.com.";
 ...
 #Check Contact Email code
  if (strcmp($code_check, $row['Contact_E_Code']) != 0) {
	  $notification2 = "We could not confirm your Contact Email. Please try reentering it from 'Change Personal Info'. If Sidney Jane continues to rage out of control, feel free to tattle to sidney.jane@forum.sandboxsalinas.com.";
...
if (isset($_POST['Wrong_Email'])) {
	 ...
	 $notification3 = "Your Contact Email is incorrect, and we cannot confirm it. Please try reentering it from 'Change Personal Info'. If Sidney Jane rages out of control, feel free to tattle to sidney.jane@forum.sandboxsalinas.com.";
...
} elseif (isset($_POST['Confirm_Email'])) {

	$set_yes= "UPDATE forum_members SET Confirmed_User='YES' WHERE user_id=" . $row['user_id'] . ";";
	
	$notification4 = "Your Contact Email has been confirmed. You can now post threads and replies. Have fun!";
	
	$confirmed_C_email = "INSERT INTO Notifications (Notification, user_id) VALUES ('" . $notification4 . "', " . $row['user_id'] . ");";
	...
	if (!mysqli_query($conn, $set_yes)) {
		...
		$unconfirmed_C_email = "INSERT INTO Notifications (Notification, user_id) VALUES ('" . $notification5 . "', " . $row['user_id'] . ");";
		
		mysqli_query($conn, $unconfirmed_C_email);
		...
		mysqli_query($conn, $confirmed_C_email);
